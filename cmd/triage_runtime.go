...
func init() {
	...
	triageCmd.Flags().StringVar(&triageProvider, "provider", workflowProviderOpenCode, "Agent provider (opencode or pi)")
	...
	triageCmd.Flags().StringVar(&triageThinking, "thinking", "", "Pi --thinking flag (off, minimal, low, medium, high, xhigh)")
	triageCmd.Flags().StringVar(&triageSession, "session", "", "Pi --session flag (resume session id)")
